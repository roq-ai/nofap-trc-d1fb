...
import { getQuotes, deleteQuoteById } from 'apiSdk/quotes';
import { QuoteInterface } from 'interfaces/quote';

type ColumnType = ColumnDef<QuoteInterface, unknown>;
...
export function QuoteListPage(props: QuoteListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getQuotes({
        ...
        order: params.order,
        searchTermKeys: ['quote_text.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<QuoteInterface>>(
    () => `/quotes?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: QuoteInterface) => {
    if (hasAccess('quote', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/quotes/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'quote_text', header: 'Quote Text', accessorKey: 'quote_text' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/quotes/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('quote', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/quotes/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('quote', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="quote" />
        </Flex>

        {hasAccess('quote', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/quotes/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
