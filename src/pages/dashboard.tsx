...
import { getDayStreaks } from 'apiSdk/day-streaks';
import { DayStreakInterface } from 'interfaces/day-streak';
import { getInspirations } from 'apiSdk/inspirations';
import { InspirationInterface } from 'interfaces/inspiration';
import { getQuotes } from 'apiSdk/quotes';
import { QuoteInterface } from 'interfaces/quote';

import { InspirationListPage as ShowCaseList } from 'pages/inspirations';

function DashboardPage() {
  const { session } = useSession();

  const dayStreakStat = useSWR<PaginatedInterface<DayStreakInterface>>(
    () => `/day-streaks`,
    () => getDayStreaks({ limit: 1, offset: 0 }),
  );

  const inspirationStat = useSWR<PaginatedInterface<InspirationInterface>>(
    () => `/inspirations`,
    () => getInspirations({ limit: 1, offset: 0 }),
  );

  const quoteStat = useSWR<PaginatedInterface<QuoteInterface>>(
    () => `/quotes`,
    () => getQuotes({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Day Streaks',
      result: dayStreakStat?.data?.totalCount,
      isLoading: dayStreakStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Inspirations',
      result: inspirationStat?.data?.totalCount,
      isLoading: inspirationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Quotes',
      result: quoteStat?.data?.totalCount,
      isLoading: quoteStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`inspirations`} passHref legacyBehavior>
                  <Button
